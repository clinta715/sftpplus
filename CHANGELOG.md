...
- **New Site Connection Bug**: Fixed connection failure for newly created sites. Added validation to reject placeholder hostnames (e.g., "new_site_1"). Fixed `save_data()` to properly re-key site data when hostname is changed from placeholder to real hostname, and updated UserRole to match.
