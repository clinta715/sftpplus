...
class MainWindow(QMainWindow):  # Inherits from QMainWindow
    ...
    def browse_ssh_key(self):
        ...
        path, _ = QFileDialog.getOpenFileName(
            ...
            filter="SSH key (*.key *.pem *.ppk);;All files (*)"
        )
        if path:
            # avoid duplicates
    ...
    def hostname_changed(self):
        ...
        if self.current_hostname in self.host_data['hostnames']:
            ...
            # Safely fetch list of key file paths (may be None or [])
            key_paths = self.host_data.get('key', {}).get(self.current_hostname) or []
            ...
            for path in key_paths:
                try:
                    ...
                    # Fallback in case basename fails
                    self.key_combo.addItem(str(path), path)
    ...
    def connect(self, hostname="localhost", username="guest", password="guest", port="22", key=None):
        ...
        try:
            ...
            if key != None:
                ...
                self.temp_key = self.key_combo.currentData()
            ...
            if not self.temp_username and not self.temp_key:
    ...
    def test_connection(self):
        ...
        try:
            ...
            ssh.connect(self.temp_hostname, port=self.temp_port, username=self.temp_username, password=self.temp_password, pkey=self.temp_key)
