...
class RemoteFileBrowser(FileBrowser):
    ...
    def sftp_getcwd(self):
        ...
        try:
            add_sftp_job(creds.get('current_remote_directory'), True, ".", True, creds.get('hostname'), creds.get('username'), creds.get('password'), creds.get('port'), "getcwd", job_id, creds.get('key'))
        except Exception as e:
            ic(e)
    ...
    def double_click_handler(self, index):
        ...
        try:
            ...
            if self.is_complete_path(filename):
                ...
                path = self.get_normalized_remote_path(creds.get('current_remote_directory'), filename)
    ...
    def upload_download(self, optionalpath=None):
        ...
        # Handle current remote directory initialization
        if creds.get('current_remote_directory') == '.':
        ...
        if current_browser is not None and isinstance(current_browser, QTableView):
            ...
            for index in indexes:
                ...
                filename = selected_item_text.split()[-1]  # Extract filename from the end
                ...
                if filename:
                    try:
                        if optionalpath is None:
                            remote_entry_path = self.get_normalized_remote_path(current_remote_directory, filename)
                        else:
                            remote_entry_path = self.get_normalized_remote_path(filename)

                        local_base_path = creds.get('current_local_directory')
                        local_entry_path = os.path.join(local_base_path, filename)

                        ic(filename, remote_entry_path, local_base_path, local_entry_path)
                        ...
                        if self.is_remote_directory(remote_entry_path):
                            ...
                            if not skip_all:
                                ...
                                ic(remote_entry_path, local_entry_path, job_id)
    ...
    def download_directory(self,
        ...
        def _handle_existing_local(local_path: str) -> str | None:
            """Return the user's choice for an already-existing local item."""
            nonlocal skip_all, overwrite_all
            if skip_all or overwrite_all or not os.path.exists(local_path):
                return None

            action = self.prompt_overwrite(local_path)
            if action in {"skip", "skip_all"}:
                skip_all = skip_all or (action == "skip_all")
                return "skip"
        ...
        try:
            ...
            entries = self.sftp_listdir(source_directory)
            ic(entries)

            processed = set()
            for name in entries:
                if name in processed:          # ignore duplicates
                    continue
                processed.add(name)

                remote_path = self.get_normalized_remote_path(source_directory, name)
                local_path  = os.path.join(destination_directory, name)
                ic(remote_path, local_path)

                try:
                    if self.is_remote_directory(remote_path):
                        action = _handle_existing_local(local_path)
                        if action == "skip":
                            continue
                        if action == "cancel":
                            return
                        self.message_signal.emit(f"Downloading directory: {remote_path} → {local_path}")
                        self.download_directory(remote_path, local_path, skip_all, overwrite_all)
                        ...
                        self.message_signal.emit(f"Downloading file: {remote_path} → {local_path}")
                        add_sftp_job(remote_path, True, local_path, False,
                                        self.init_hostname, self.init_username,
                                        self.init_password, self.init_port,
                                        "download", job_id, self.init_key)

                except Exception as e:
                    self.message_signal.emit(f"Error processing {remote_path}: {e}")
                    ic(e)

        except Exception as e:
            self.message_signal.emit(f"download_directory() error: {e}")
            ic(e)
        finally:
            self.notify_observers()
