...
# Backward compatibility: keep add_sftp_job working
def add_sftp_job_legacy(source_path, is_source_remote, destination_path, 
                        is_destination_remote, hostname, username, password, 
                        port, command, job_id, key):
    """
    Legacy add_sftp_job function for backward compatibility.
    
    This function creates a session internally and executes the command.
    It provides the same interface as the original add_sftp_job.
    
    Args:
        source_path: Source file path
        is_source_remote: True if source is remote
        destination_path: Destination file path
        is_destination_remote: True if destination is remote
        hostname: SFTP hostname
        username: SSH username
        password: SSH password
        port: SSH port
        command: Command type ('download', 'upload', 'listdir', etc.)
    ...
    if command == "download":
        api.download(source_path, destination_path, job_id)
    elif command == "upload":
        api.upload(source_path, destination_path, job_id)
    elif command == "listdir":
        api.list(source_path, job_id)
    elif command == "listdir_attr":
        api.list_attr(source_path, job_id)
    elif command == "stat":
        api.stat(source_path, job_id)
    elif command == "mkdir":
        api.mkdir(source_path, job_id)
    elif command == "rmdir":
        api.rmdir(source_path, job_id)
    elif command == "remove":
        api.remove(source_path, job_id)
    elif command == "chdir":
        api.chdir(source_path, job_id)
    else:
