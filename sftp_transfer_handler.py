...
class TraversalWorker(QRunnable):
    ...
    def __init__(self, session_id, source_dir, dest_dir, is_source_remote, is_dest_remote,
        ...
        # SFTPOperations for listing - use discovery channel for faster traversal
        from sftp_connection_pool import get_connection_pool
        ...
        if discovery_ssh and discovery_sftp:
            ...
            self._discovery_api = SFTPSessionAPI(self._discovery_session, 
                                                 ssh=discovery_ssh, 
                                                 sftp=discovery_sftp)
            self._discovery_ssh = discovery_ssh
            self._discovery_sftp = discovery_sftp
            self._discovery_channel = True
            # Use the session API for operations
    ...
    def run(self):
        ...
        def execute_cmd(cmd_type, *args, **kwargs):
            if self._discovery_api:
                if cmd_type == 'exec_command':
                    # Use discovery SSH directly for find command
                    return self._discovery_ssh.exec_command(*args, **kwargs)
                elif cmd_type == 'list':
